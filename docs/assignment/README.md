...
The application built from these sources keeps pure analytical rules in `src/domain/`, shared contracts and the runtime-neutral SQL port in `src/shared/`, data/bootstrap utilities in `src/data/`, and Cloudflare Worker adapters in `src/server/`. The original files listed above remain supplied inputs and are not modified by that organization.
...
- [Implementation plan](../architecture/implementation-plan.md): small submission scope and future work.
